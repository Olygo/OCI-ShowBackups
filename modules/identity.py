...
##########################################################################
# check if compartment exists, else create it
##########################################################################

def check_compartment(identity, tenancy_id, target_comp):
    ...
    for compartment in all_compt.data:
        if compartment.name == target_comp:
            print(green+f"{'*'*5:5} {'Compartment:':15} {'found':20} {compartment.id:40} {'*'*5:5}")
            target_comp_id = compartment.id

    if len(target_comp_id) < 1:
        print(yellow+f"{'*'*5:5} {'Compartment:':15} {'creating':20} {target_comp:40} {'*'*5:5}")

        result = identity.create_compartment(oci.identity.models.CreateCompartmentDetails(
            compartment_id = tenancy_id,
            description = 'oci backup reports',
            name = target_comp))
        ...
        print(green+f"{'*'*5:5} {'Compartment:':15} {'created':20} {target_comp_id:40} {'*'*5:5}")
